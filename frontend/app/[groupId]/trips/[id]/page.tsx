...
export default function TripDetailPage() {
  ...
  const [activeItineraryTab, setActiveItineraryTab] =
    useState<ItineraryTab>("schedule");
  ...
  useEffect(() => {
    const fetchTrip = async () => {
      try {
        ...
        setUser(meRes.data);
        const [tripRes, scheduleRes, transportRes, lodgingRes, budgetRes] =
          await Promise.all([
        ...
        setOverviewNotify(
          tripRes.data.notify_at ? toDateInput(tripRes.data.notify_at) : "",
        );
  ...
  const saveTransports = async () => {
    ...
    const transportRes = await api.get<TransportResponseItem[]>(
      `/trips/${trip.id}/transports`,
    );
  ...
  const saveOverview = async () => {
    ...
    try {
      ...
      setOverviewNotify(
        res.data.notify_at ? toDateInput(res.data.notify_at) : "",
      );
    } catch (err) {
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="card">
          <TripHeader
            ...
            onPost={() =>
              router.push(
                `/${groupIdParam}/posts/new?trip_id=${trip.id}&redirect=/${groupIdParam}/trips/${trip.id}`,
              )
            }
            onAlbum={() =>
              router.push(
                `/${groupIdParam}/albums/new?trip_id=${trip.id}&redirect=/${groupIdParam}/trips/${trip.id}`,
              )
            }
          />
                ...
                <nav className="-mb-px flex flex-wrap gap-4">
                  {[
                    ...
                    <button
                      key={tab.key}
                      onClick={() =>
                        handleItineraryTabSwitch(
                          tab.key as typeof activeItineraryTab,
                        )
                      }
                      className={`border-b-2 py-2 text-sm font-medium ${
