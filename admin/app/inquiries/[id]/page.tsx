...
export default function AdminInquiryDetailPage() {
  ...
  const handleReply = async () => {
    ...
    try {
      ...
      await api.post(`/admin/inquiries/${detail.id}/replies`, {
          ...
          <>
            ...
            <div className="card space-y-3">
              <div className="flex flex-wrap items-center gap-3 text-sm text-gray-600">
                <span
                  className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${statusBadgeClass(detail.status)}`}
                >
                  {statusLabel(detail.status)}
                </span>
                <span>受付日時: {formatDateTime(detail.created_at)}</span>
                {detail.thread_id && (
                  <span>スレッドID: {detail.thread_id}</span>
                )}
              </div>
