...
export default function AdminInquiriesPage() {
  ...
  const [statusFilters, setStatusFilters] = useState<InquiryStatus[]>(['pending', 'in_progress'])
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((item) => {
      ...
      const userText = [item.user_name, item.user_email, item.contact_name, item.contact_email, item.user_id]
        .filter(Boolean)
        .join(' ')
        .toLowerCase()
  ...
  return (
    <div className="min-h-screen admin">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div>
          <h2 className="text-2xl font-semibold text-gray-800">お問い合わせ一覧</h2>
          <p className="text-gray-600 mt-1">送信されたお問い合わせの内容を確認できます</p>
        </div>

        <div className="card">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-6">
            <div className="space-y-1">
              <label htmlFor="inquiry-user-filter" className="block text-sm font-medium text-gray-700">
                ユーザー
              </label>
              <input
            ...
            <div className="space-y-1">
              <span className="block text-sm font-medium text-gray-700">対応状況</span>
            ...
            <div className="space-y-1">
              <label htmlFor="inquiry-category-filter" className="block text-sm font-medium text-gray-700">
                カテゴリ
              </label>
              <select
            ...
            <div className="space-y-1">
              <span className="block text-sm font-medium text-gray-700">表示順</span>
              <button
                type="button"
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-200">
                    <th className="text-left py-3 px-2 font-medium text-gray-600 min-w-[160px]">受信日時</th>
                    <th className="text-left py-3 px-2 font-medium text-gray-600 min-w-[100px]">対応状況</th>
                    <th className="text-left py-3 px-2 font-medium text-gray-600 min-w-[120px]">カテゴリ</th>
                    <th className="text-left py-3 px-2 font-medium text-gray-600 min-w-[200px]">件名</th>
                    <th className="text-left py-3 px-2 font-medium text-gray-600 min-w-[200px]">ユーザー</th>
                    <th className="text-left py-3 px-2 font-medium text-gray-600 min-w-[80px]">操作</th>
                  </tr>
                </thead>
                <tbody>
                  {sortedItems.map((item) => (
                    <tr key={item.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                      ...
                      <td className="py-3 px-2">
                        <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${statusBadgeClass(item.status)}`}>
                          {statusLabel(item.status)}
                        </span>
                      </td>
                      <td className="py-3 px-2 text-gray-700">{item.category || '-'}</td>
                      <td className="py-3 px-2 text-gray-900">{item.subject || '-'}</td>
                      <td className="py-3 px-2">
                        <div className="text-gray-900">{item.contact_name || item.user_name || item.user_id || '-'}</div>
                        <div className="text-xs text-gray-500">{item.contact_email || item.user_email || '-'}</div>
                      </td>
                      <td className="py-3 px-2">
                        <Link
